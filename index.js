...
function filterByGenre(movies, genre) {
  ...
  result = movies.filter(movie => movie.genre.toLowerCase().includes(genre.toLowerCase()))
  return result
}
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  result = movies.filter(movie => Number(movie.released.slice(-4)) <= year)
  return result
}
...
function getRottenTomatoesScoreByMovie(movies) {
  ...
  return movies.map(movie => {
    let result = {};
    result[movie.title] = movie.ratings.find(rating => rating.source === "Rotten Tomatoes").value
    return result
  })
  }
